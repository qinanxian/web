...
;(function($w){
    ...
    // core object
    var jm = function(options, setIcon){
        jm.current = this;
    ...
    jm.mind.prototype = {
        ...
        _move_node:function(node, beforeid, parentid, direction){
            if(!!node && !!parentid){
                if(node.parent.id != parentid){
                    ...
                    // 重新计算id的值-jkwu.2019-05-06;20:30
                    node.parent = this.get_node(parentid);
                    const nodeId = node.id;
                    const pathArrays = node.data.path.split('.') || [];
                    const contextId = pathArrays.pop();
                    const tempNodePath = `${node.parent.data.path}.${contextId}`;
                    node.data = {
                      ...node.data,
                      path: tempNodePath,
                      contextId: contextId
                    ...
                    if (node.children.length) {
                        node.children = this.set_children_path(node);
                    }
                    this.change_node_path(nodeId, tempNodePath);
        ...
        set_children_path: function (node, view) {
            return node.children.map(item => {
                item.data.path = `${item.parent.data.path}.${item.data.contextId}`;
                const tempNodePath = `${item.parent.data.path}.${item.data.contextId}`;
                this.change_node_path(item.id, tempNodePath);
                let tempChildren = item.children;
                if (tempChildren && tempChildren.length) {
                    tempChildren = this.set_children_path(item, view);
        ...
        change_node_path: function (nodeId, nodePath) {
          // self-achive-jkwu
          if(!(nodeId in this.nodes)) {
            logger.warn('the nodeid \''+ nodeId +'\' has not exist.');
            return false;
          } else {
            this.nodes[nodeId].data.path = nodePath;
            return true;
          }
        },

        is_exsit_id: function (id) {
            // self-achive-jkwu
        ...
        node_tree:{
            ...
            _extract_data:function(node_json){
                ...
                for(var k in node_json){
                    // 2019-5-6 14:24注释掉获取id属性
                    if(k == 'id' || k=='topic' || k=='children' || k=='direction' || k=='expanded'){
                    // if(k=='topic' || k=='children' || k=='direction' || k=='expanded'){
                        continue;
                    }
                    data[k] = node_json[k];
                }
    ...
    jm.prototype={
        ...
        update_node:function(nodeid, topic, flag){
            if(this.get_editable()){
        ...
        move_node:function(nodeid, beforeid, parentid, direction){
            if(this.get_editable()){
                ...
                if(!!node){
                    this.view.update_node({ ...node, id: nodeid });
                    this.layout.layout();
                    this.view.show(false);
...
// ============= data provider =============================================
    ...
    jm.view_provider.prototype={
        ...
        init_nodes_size:function(node){
            var view_data = node._data.view;
            var width = view_data.element.clientWidth;
            if (view_data.element.children && view_data.element.children.length > 0) {
                width += 2;
            }
        ...
        create_node_element:function(node,parent_node){
            ...
            if (!!node.topic) {
                if(this.opts.support_html){
                    this.setIcon(d, node.topic, node.data.icon);
                }else{
                    $t(d,node.topic);
                }
            }
            ...
            // 判断子节点，用于控制菜单样式
            const isHasChildren = node.children.length > 0;
            this._reset_node_custom_style(d, node.data, isHasChildren);
        ...
        update_node:function(node){
            ...
            if (!!node.topic) {
                if(this.opts.support_html){
                    this.setIcon(element,node.topic, node.data.icon);
                }else{
        ...
        set_multiple_select_node: function(selectedIds) {
            // self-jkwu-2019-06-05
            var nodes = this.jm.mind.nodes;
            selectedIds.map(id => {
                const node = nodes[id];
                if(!!node){
        ...
        clear_multiple_select_node: function(selectedIds) {
            // self-jkwu-2019-06-05
            var nodes = this.jm.mind.nodes;
            selectedIds.map(id => {
                const node = nodes[id];
                if(!!node){
                    node._data.view.element.className =
                      node._data.view.element.className.replace(/\s*selected\b/i,'');
                    this.reset_node_custom_style(this.selected_node);
                }
            });
        },
        ...
        show_nodes:function(){
            ...
            for(var nodeid in nodes){
                ...
                if(!node.isroot && node.children.length>0){
                    ...
                    expander.style.left = (_offset.x + p_expander.x + 2) + 'px';
        ...
        reset_node_custom_style:function(node){
          // 判断子节点，用于控制菜单样式
          const isHasChildren = node.children.length > 0;
            this._reset_node_custom_style(node._data.view.element, node.data, isHasChildren);
        },

        _reset_node_custom_style:function(node_element, node_data, isHasChildren){
            const nodeData = node_data;
            if (nodeData && nodeData.type === 'menu') {
              // 设置menu的样式
              if (!nodeData.url && isHasChildren === false) {
                // 判断，如果是menu节点并且没有子节点的情况，如果URL为空，让节点适度变灰
                node_element.setAttribute('class', 'jmnode-menu-disabled');
              } else {
                node_element.setAttribute('class', 'jmnode-menu');
              }
            } else if (nodeData && nodeData.type === 'action') {
              node_element.setAttribute('class', 'jmnode-action');
            } else if (nodeData && nodeData.type === 'datascope') {
              node_element.setAttribute('class', 'jmnode-datascope');
            }
            // 设置样式
    ...
    jm.shortcut_provider.prototype = {
        ...
        handle_addchild: function(_jm,e){
            ...
            if(!!selected_node){
                ...
                var node = _jm.add_node(selected_node, nodeid, '新增节点');
                if(!!node){
        ...
        handle_addbrother:function(_jm,e){
            ...
            if(!!selected_node && !selected_node.isroot){
                var nodeid = jm.util.uuid.newid();
                var node = _jm.insert_node_after(selected_node, nodeid, '新增节点');
                if(!!node){
                    _jm.select_node(nodeid);
                    _jm.begin_edit(nodeid);
                }
