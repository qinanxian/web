...
(function($w){
    ...
    jsMind.screenshot.prototype = {
        ...
        shootDownload: function(imageType){
            this.shoot(function(jms){
                jms._download(imageType);
            });
        },
        ...
        _watermark:function(){
            ...
            ctx.fillText(`Amix-${moment(new Date(), 'YYYY/MM/DD HH:mm').format('YYYY/MM/DD HH:mm')}`,c.width-5.5,c.height-2.5);
            // ctx.textAlign='left';
        ...
        _download:function(imageType = 'png'){
            var c = this.canvas_elem;
            var name = `menu_${moment(new Date(), 'YYYY_MM_DD').format('YYYY_MM_DD')}.${imageType}`;

            if (navigator.msSaveBlob && (!!c.msToBlob)) {
                var blob = c.msToBlob();
                navigator.msSaveBlob(blob,name);
            } else {
                var bloburl = this.canvas_elem.toDataURL(`image/${imageType}` , 1.0);
