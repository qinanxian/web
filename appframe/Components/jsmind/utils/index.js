...
export function addProperties(object, shortObject, util) {
  let newObject = {};
  if (typeof object === 'object') {
    newObject = {
      ...object,
    ...
    if ('children' in object) {
      ...
      if (object.children&&object.children.length) {
        children = object.children
          .map(item => addProperties(item, shortObject, util));
      }
...
export function removeProperties(object, removeKeys) {
  let newObject = {};
  const needRemoveKeys = removeKeys || [];
  if (typeof object === 'object') {
    const objectKeys = Object.keys(object);
    if (objectKeys.length) {
      objectKeys.map((tempKey) => { // eslint-disable-line
        // 如果是需要移除的key，则不作处理,
        if (!(needRemoveKeys.includes(tempKey))) {
          if (tempKey === 'children') {
            newObject[tempKey] =
              object[tempKey].map(childrenItem => removeProperties(childrenItem, needRemoveKeys));
          } else {
            newObject[tempKey] = object[tempKey];
          }
        }
